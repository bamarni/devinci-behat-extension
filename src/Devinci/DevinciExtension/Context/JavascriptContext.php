...
class JavascriptContext extends RawMinkContext
{
     ...
     */
    public function spin($lambda, $wait = -1)
    {
     ...
     */
    public function iWaitFor($text, $wait = -1)
    {
        $wait = ($wait < 0) ? $this->maximum_wait : $wait;
        try {
            $found = $this->spin(function ($context) use ($text) {$this->assertSession()->pageTextContains($this->fixStepArgument($text));return true;}, $wait);
            return $found;
        } catch (\Exception $e) {
     ...
     */
    public function findWithTimeout($search, $selector = 'css', $wait = -1)
    {
        $wait = ($wait < 0) ? $this->maximum_wait : $wait;
